<?php
function obtener_productos_paginados($conn, $ventas_por_pagina, $pagina) {
    $offset = ($pagina - 1) * $ventas_por_pagina;
    $consulta = $conn->prepare("SELECT * FROM tventas LIMIT ?, ?");
    $consulta->bind_param('ii', $offset, $ventas_por_pagina);
    $consulta->execute();
    return $consulta->get_result();
}

function obtener_total_productos($conn) {
    $resultado = $conn->query("SELECT COUNT(*) as total FROM tventas");
    return $resultado->fetch_assoc()['total'];
}

function generar_enlaces_paginacion($total_registros, $ventas_por_pagina, $pagina_actual) {
