<?php
function obtener_productos_paginados($conn, $productos_por_pagina, $pagina) {
    $offset = ($pagina - 1) * $productos_por_pagina;
    $consulta = $conn->prepare("SELECT * FROM tprodu LIMIT ?, ?");
    $consulta->bind_param('ii', $offset, $productos_por_pagina);
    $consulta->execute();
...
function generar_enlaces_paginacion($total_registros, $productos_por_pagina, $pagina_actual) {
